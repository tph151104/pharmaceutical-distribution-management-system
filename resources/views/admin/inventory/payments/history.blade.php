...
@section('content')
            ...
            <form action="{{ route('payments.history') }}" method="GET" class="row g-2 align-items-end">
                ...
                <div class="col-md-3">
                    <label class="form-label small fw-medium">
                        {{ $tab === 'tra_hang' ? 'Tìm mã GD / mã đơn trả' : 'Tìm mã giao dịch / mã phiếu' }}
                    </label>
                    <input type="text" name="search" class="form-control form-control-sm" value="{{ $search }}"
                           placeholder="{{ $tab === 'tra_hang' ? 'Nhập mã đơn trả...' : 'Nhập mã...' }}">
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-2 d-flex gap-2 justify-content-end">
                    ...
                    <a href="{{ route('payments.history.export', ['tab' => $tab, 'search' => $search, 'from_date' => $fromDate, 'to_date' => $toDate]) }}"
                       target="_blank" class="btn btn-sm btn-success flex-grow-1" title="Xuất Excel">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body p-0">
                ...
                @if($tab === 'tra_hang')
                            ...
                            @forelse($transactions as $maTraHang => $groupTx)
                                ...
                                <tr class="table-secondary border-bottom">
                                    ...
                                    <td class="pe-3 text-end">
                                        <button class="btn btn-sm btn-outline-dark" type="button"
                                                ...
                                                data-bs-target="#collapse-{{ \Str::slug($maTraHang) }}">
                                ...
                                <tr class="p-0 border-0">
                                    <td colspan="5" class="p-0 border-0">
                                        <div class="collapse" id="collapse-{{ \Str::slug($maTraHang) }}">
                                            <table class="table table-sm table-bordered mb-0 bg-light w-100">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="fw-medium text-warning small">{{ $tx->ma_thanh_toan }}</td>
                                                        <td class="small">{{ $tx->phuong_thuc_tt }}</td>
                                                        <td class="text-end fw-bold small text-warning">{{ number_format($tx->so_tien_tt) }}</td>
                                                        <td class="text-center pe-3">
                                                            <a href="{{ route('payments.show', $tx->ma_thanh_toan) }}" target="_blank"
                                                               class="btn btn-xs btn-outline-warning py-0" style="font-size: 0.8rem;">
                    ...
                    @else
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-4 text-muted">Không có lịch sử hoàn tiền nào phù hợp.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    @endif
                            ...
                            @forelse($transactions as $maPhieu => $groupTx)
                                ...
                                <tr class="table-secondary border-bottom">
                                    ...
                                    <td class="pe-3 text-end">
                                        <button class="btn btn-sm btn-outline-dark" type="button"
                                                data-bs-toggle="collapse"
                                                data-bs-target="#collapse-{{ \Str::slug($maPhieu) }}">
                                            <i class="bi bi-chevron-down"></i> Xem chi tiết
                                        </button>
                                    </td>
                                </tr>
                                            ...
                                            <table class="table table-sm table-bordered mb-0 bg-light w-100">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-center pe-3">
                                                            <a href="{{ route('payments.show', $tx->ma_thanh_toan) }}" target="_blank"
                                                               class="btn btn-xs btn-outline-primary py-0" style="font-size: 0.8rem;">
                                                                <i class="bi bi-printer"></i> In HĐ
                                                            </a>
                                                        </td>
                                                    </tr>
            ...
            @if($groupBy != 'true' && method_exists($transactions, 'hasPages') && $transactions->hasPages())
